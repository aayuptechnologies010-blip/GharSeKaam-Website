...
import {
  Droplets, Layers, Flame, Wrench, Construction,
  Phone, MessageCircle, Sparkles, Shield, Star,
...
const SERVICES_DATA: ServiceItem[] = [
  {
    ...
    features: ["Deep-boring up to 400+ feet", "High-grade GI & PVC casing pipes", "Submersible pump matching & setup", "1 Year operational warranty"]
  },
  {
    id: "soil-filling",
    ...
    features: ["River silt/clay soil delivery in bulk", "Tractor leveling & grader leveling", "Compaction with heavy vibratory rollers", "Precise site alignment and mapping"]
  },
  {
    id: "brick-kiln",
    ...
    features: ["Grade-A solid red clay bricks", "Bulk dumper dispatch (2000-5000 units)", "Direct kiln wholesale rates", "Minimal breakage handling"]
  },
  {
    id: "septic-clean",
    ...
    features: ["High-power vacuum pumping tankers", "Blockage extraction with sewer rods", "High-pressure jet washing", "Eco-friendly waste disposal"]
  },
  {
    id: "roof-casting",
    ...
    features: ["Plywood/MS structural shuttering", "Rebar placement & structural verification", "Mechanized concrete mixture casting", "Waterproofing additives integration"]
  }
];

const ContractingOfferings = () => {
  const [selectedService, setSelectedService] = useState<ServiceItem | null>(null);

  return (
    <div className="py-6">
      {/* Trust Strip */}
      <div className="bg-[#eaeded] py-4 border-b border-slate-200 mb-10">
      ...
      <div className="container mx-auto px-6 max-w-5xl">
        <div className="text-center space-y-2.5 mb-14">
          <div className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-amber-100 border border-amber-300/50 text-amber-700 text-[10px] font-black uppercase tracking-widest">
                ...
                <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                  <div className="space-y-2 text-left">
                    ...
                    <h3 className="text-base font-black text-slate-800 leading-tight">
                      {service.title}
                      <span className="block text-xs font-semibold text-slate-400 mt-0.5">{service.titleHi}</span>
                    </h3>
                    <p className="text-xs text-slate-500 font-semibold leading-relaxed">{service.description}</p>
                  </div>

                  <div className="pt-3 border-t flex items-center justify-between">
                    <span className="text-[10px] text-amber-600 font-black uppercase tracking-wider">{service.rate}</span>
                    <Button size="sm" className="bg-slate-900 hover:bg-amber-500 hover:text-slate-950 text-white font-extrabold text-[10px] uppercase rounded-xl h-8 px-4">
          ...
          <DialogContent className="sm:max-w-[500px] rounded-3xl bg-white border border-slate-200 overflow-hidden shadow-2xl p-0 [&>button]:top-5 [&>button]:right-5 [&>button]:bg-slate-100 [&>button]:hover:bg-slate-200 [&>button]:p-1.5 [&>button]:rounded-full [&>button]:transition-all">
            ...
            <div className="p-6 md:p-8 space-y-6">
              <DialogHeader className="text-left space-y-2">
                <div className="flex items-start gap-3 pt-2">
                  ...
                  <div className="space-y-1">
                    <DialogTitle className="text-lg font-black text-slate-800 leading-snug">{selectedService.title}</DialogTitle>
                    <span className="block text-xs font-bold text-amber-600">{selectedService.titleHi}</span>
                  </div>
                </div>
              </DialogHeader>

              <div className="space-y-4 text-left">
                <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 text-xs font-semibold text-slate-600 leading-relaxed">
              ...
              <DialogFooter className="border-t pt-4">
                <Button onClick={() => setSelectedService(null)} className="w-full bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold rounded-xl">
                  Close
                </Button>
              </DialogFooter>
            </div>
...
export default ContractingOfferings;
